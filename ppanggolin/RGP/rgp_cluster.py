...
def add_info_to_identical_rgps(rgp_graph: nx.Graph, identical_rgps_objects: List[IdenticalRegions],
...
def add_edges_to_identical_rgps(rgp_graph: nx.Graph, identical_rgps_objects: List[IdenticalRegions]):
    ...
    for identical_rgp_obj in identical_rgps_objects:
        ...
        rgp_graph.add_nodes_from([ident_rgp.ID for ident_rgp in identical_rgp_obj.rgps],
...
def dereplicate_rgp(rgps: Set[Union[Region, IdenticalRegions]],
                    disable_bar: bool = False) -> List[Union[Region, IdenticalRegions]]:
    """
...
def compute_rgp_metric(rgp_a: Region,
                       rgp_b: Region,
                       grr_cutoff: float,
                       grr_metric: str) -> Union[Tuple[int, int, dict], None]:
    """
